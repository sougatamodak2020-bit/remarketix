...
import {
  ArrowRight, Check, TrendingUp, Target, Palette, Search,
  LayoutTemplate, Megaphone, Sparkles, Zap, Shield, Clock,
  Star, Rocket, Award, ChevronLeft, ChevronRight
} from "lucide-react";
...
const PARTICLE_POSITIONS = [
  { left: 15.2, top: 23.4, color: 0 }, { left: 78.3, top: 67.1, color: 1 },
  { left: 42.7, top: 89.3, color: 2 }, { left: 91.5, top: 12.8, color: 0 },
  { left: 23.8, top: 45.6, color: 1 }, { left: 67.2, top: 78.9, color: 2 },
  { left: 34.9, top: 34.2, color: 0 }, { left: 56.4, top: 91.7, color: 1 },
  { left: 12.1, top: 56.3, color: 2 }, { left: 88.6, top: 41.8, color: 0 },
];
...
function SwipeCarousel({ children, className = "" }: { children: React.ReactNode[]; className?: string }) {
  ...
  return (
    <div className={`relative ${className}`}>
      <div className="overflow-hidden" onTouchStart={onTouchStart} onTouchEnd={onTouchEnd}>
        ...
        >
          {children.map((child, i) => (
            <div key={i} className="min-w-full px-4" style={{ flex: "0 0 100%" }}>{child}</div>
          ))}
        </div>
      </div>
      {/* Dots */}
      ...
      {idx > 0 && (
        <button onClick={prev} className="absolute left-0 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 border border-white/20 rounded-full flex items-center justify-center touch-manipulation">
          <ChevronLeft className="w-5 h-5 text-white" />
        </button>
      )}
      {idx < total - 1 && (
        <button onClick={next} className="absolute right-0 top-1/2 -translate-y-1/2 w-10 h-10 bg-white/10 border border-white/20 rounded-full flex items-center justify-center touch-manipulation">
...
export default function HomeView() {
  ...
  const isStandardInView = useInView(standardRef, { once: true, margin: "-80px" });

  // PERF: useScroll + useSpring removed entirely — was the #1 frame-rate killer
  ...
  const capabilities = useMemo(() => [
    {
      icon: Search, gradient: "from-emerald-500 via-emerald-400 to-teal-400",
      title: "Data & Research",
    ...
    {
      icon: Palette, gradient: "from-blue-500 via-blue-400 to-cyan-400",
      title: "Content & SEO",
    ...
    {
      icon: LayoutTemplate, gradient: "from-violet-500 via-purple-400 to-fuchsia-400",
      title: "Web & Design",
    ...
    {
      icon: Megaphone, gradient: "from-orange-500 via-amber-400 to-yellow-400",
      title: "Advertising",
  ...
  return (
    <div className="bg-[var(--bg-primary)] text-white overflow-hidden">
      ...
      <section ref={heroRef} className="relative min-h-screen flex items-center justify-center overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <div
            ...
            style={{
              background: "radial-gradient(ellipse 50% 50% at 50% 50%, rgba(16,185,129,0.18), rgba(6,182,212,0.13) 40%, transparent 70%)",
              filter: "blur(70px)",
            }}
          />
          {isClient && !isMobile && (
            <div
              ...
              style={{
                background: "radial-gradient(ellipse 50% 50% at 50% 50%, rgba(59,130,246,0.13), rgba(139,92,246,0.08) 40%, transparent 70%)",
                filter: "blur(90px)",
              }}
            />
          )}
        ...
        {/* Hero content — PERF: no style={{ y, opacity, scale }} motion props (removed parallax) */}
        <div className="relative z-10 container-custom pt-24 md:pt-32 pb-16 md:pb-20 px-4">
          ...
          >
            ...
            {/* Heading */}
            <motion.h1 variants={fadeInUp} className="heading-display mb-8 md:mb-10 relative text-4xl md:text-6xl lg:text-7xl">
              <motion.span
              ...
              >
                {/* PERF: CSS animation via inline animationName instead of Framer animate */}
                <span
                  ...
                  style={{
                    ...
                    animationName: (!shouldReduceMotion && !isMobile) ? "gradient-shift" : "none",
            ...
            {/* Subtitle */}
            <motion.p variants={fadeInUp} className="text-base md:text-lg lg:text-xl max-w-3xl mb-10 md:mb-14 text-white/80 leading-relaxed px-4">
            ...
            {/* CTA buttons */}
            <motion.div variants={fadeInUp} className="flex flex-col sm:flex-row gap-4 md:gap-5 mb-12 md:mb-16 w-full sm:w-auto px-4">
              <motion.button
            ...
            {/* Trust badges */}
            <motion.div variants={fadeInUp} className="flex flex-wrap items-center justify-center gap-4 md:gap-6 mb-16 md:mb-20 px-4">
            ...
            <motion.div variants={fadeInUp} className="w-full max-w-5xl px-4">
              {isMobile ? (
                <SwipeCarousel>
                  {stats.map((stat, i) => (
                    <div key={i} className="stat-card-premium mx-auto max-w-sm">
      ...
      <section ref={standardRef} className="section-spacing relative overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          {isClient && !isMobile && (
            <div className="absolute top-1/4 right-1/4 w-96 h-96 rounded-full"
              style={{ background: "radial-gradient(circle, rgba(6,182,212,0.08), transparent 70%)", filter: "blur(80px)" }}
            />
          )}
        </div>
        ...
        <div className="container-custom relative z-10 px-4">
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 xl:gap-28 items-center">
            ...
            >
              <motion.div className="badge mb-6 md:mb-8" whileHover={!isMobile ? { scale: 1.05 } : {}}>
              ...
              <h2 className="heading-xl mb-6 md:mb-8 text-3xl md:text-5xl lg:text-6xl">
                ...
                <motion.span className="block mt-2 md:mt-3 relative inline-block"
                  initial={{ opacity: 0, y: 20 }}
                  animate={isStandardInView ? { opacity: 1, y: 0 } : {}}
              ...
              <motion.p className="text-base md:text-lg lg:text-xl mb-10 md:mb-14 leading-relaxed text-white/75"
                initial={{ opacity: 0 }}
              ...
              <div className="grid gap-6 md:gap-8">
                {[
                  { icon: Target,     color: "emerald", title: "Precision",  desc: "ICP-based targeting that reaches real decision-makers"    },
                  { icon: Palette,    color: "blue",    title: "Creativity", desc: "Modern design and compelling creatives that convert"      },
                  { icon: TrendingUp, color: "violet",  title: "Conversion", desc: "Sales-focused funnels that turn traffic into revenue"     },
                ].map((item, i) => (
                  <motion.div key={item.title} className="group relative"
                    ...
                    <div className="relative flex items-start gap-4 md:gap-6 p-4 md:p-6 rounded-2xl bg-white/[0.02] border border-white/[0.05] transition-colors duration-300 group-hover:border-white/10 group-hover:bg-white/[0.04]">
                      ...
                      {!isMobile && (
                        <div className={`opacity-0 group-hover:opacity-100 transition-opacity text-${item.color}-400 mt-1`}>
            ...
            >
              ...
              {!isMobile && (
                <div className="absolute -inset-6 rounded-[2rem]"
                  style={{ background: "linear-gradient(135deg,rgba(16,185,129,0.14),rgba(6,182,212,0.09),rgba(59,130,246,0.1))", filter: "blur(40px)" }}
                />
              )}

              <div className="relative rounded-2xl md:rounded-[2rem] overflow-hidden border border-white/10 shadow-2xl">
                <motion.div whileHover={!isMobile ? { scale: 1.04 } : {}} transition={{ duration: 0.5 }}>
                  <Image
                ...
                {/* Badge overlay */}
                <motion.div className="absolute bottom-0 left-0 right-0 p-4 md:p-8"
              ...
              {!isMobile && (
                <>
                  <motion.div className="absolute -right-6 top-1/4 bg-black/60 border border-emerald-500/30 rounded-2xl p-4 shadow-2xl"
                  ...
                  <motion.div className="absolute -left-6 bottom-1/3 bg-black/60 border border-cyan-500/30 rounded-2xl p-4 shadow-2xl"
                    initial={{ opacity: 0, x: -20 }}
                    animate={isStandardInView ? { opacity: 1, x: 0 } : {}}
        ...
        <div className="absolute inset-0 pointer-events-none">
          ...
          {isClient && !isMobile && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full h-full"
              style={{ background: "radial-gradient(circle 700px at 50% 50%, rgba(139,92,246,0.04), transparent)" }}
            />
          )}
        </div>
      ...
      {/* ================================================================
          ADVANTAGES — "Your unfair advantage"
          ================================================================ */}
      <section className="section-spacing relative overflow-hidden">
        {/* PERF: 3 static blobs instead of 3 animated orbiting ones */}
        <div className="absolute inset-0 pointer-events-none">
        ...
        <div className="container-custom relative z-10 px-4">
          ...
          >
            ...
            <h2 className="heading-xl mb-4 md:mb-6 text-3xl md:text-5xl lg:text-6xl">
              ...
              <span className="gradient-text-enhanced block mt-2 md:mt-3">Data, Web &amp; Demand</span>
            </h2>
            <motion.div
            ...
            {advantages.map((adv, i) => (
              <motion.div key={adv.title} className="group relative"
                ...
                <motion.div
                  ...
                  {/* PERF: CSS-only gradient overlay (no motion.div) */}
                  ...
                  <div className="relative z-10">
                    <motion.div
                      ...
                      transition={{ duration: 0.7 }}>
                      <adv.icon className={`w-6 h-6 md:w-8 md:h-8 text-${adv.color}-400`} />
                    </motion.div>
                    ...
                    {!isMobile && (
                      <div className={`absolute top-6 right-6 opacity-0 group-hover:opacity-100 transition-opacity text-${adv.color}-400`}>
                        <ArrowRight className="w-5 h-5" />
                      </div>
                    )}
      ...
      {/* ================================================================
          ...
          ================================================================ */}
      <section className="section-spacing relative overflow-hidden">
        {/* PERF: static background — removed animated conic gradient + animated orb */}
        <div className="absolute inset-0 pointer-events-none">
          {isClient && !isMobile && (
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-[900px] h-[500px]"
              style={{
                background: "radial-gradient(ellipse 50% 50% at 50% 50%,rgba(16,185,129,0.13),rgba(6,182,212,0.09) 30%,rgba(59,130,246,0.06) 60%,transparent 80%)",
                filter: "blur(80px)",
              }}
            />
          )}
        ...
        <div className="container-custom relative z-10 px-4">
          <motion.div
            ...
            <div className="card-glass-premium text-center p-8 md:p-12 lg:p-20 relative overflow-hidden">
              ...
              {/* PERF: static border gradient instead of animated */}
              <div className="absolute inset-0 rounded-2xl md:rounded-[2rem] pointer-events-none"
                style={{
              ...
              <div className="relative z-10">
                ...
                <motion.p
                  ...
                  <span className="text-blue-400 font-semibold">better design</span>, and{" "}
              ...
              {/* PERF: static decorative blobs instead of animated ones */}
              {isClient && !isMobile && (
                <>
                  <div className="absolute top-10 right-10 w-20 h-20 rounded-full bg-gradient-to-br from-emerald-400/12 to-cyan-400/12 blur-2xl pointer-events-none" />
                  <div className="absolute bottom-10 left-10 w-32 h-32 rounded-full bg-gradient-to-br from-blue-400/12 to-violet-400/12 blur-2xl pointer-events-none" />
                </>
              )}
            </div>
          </motion.div>
  ...
  return (
    ...
    >
      <motion.div
        ...
        transition={{ duration: 0.35 }}
      >
        {/* Background image — PERF: CSS scale via group-hover instead of motion.div wrapper */}
        ...
        <div className={`absolute inset-0 bg-gradient-to-b ${cap.gradient} opacity-0 group-hover:opacity-30 transition-opacity duration-500`} />

        {/* Border glow */}
        <div
          className="absolute inset-0 rounded-2xl md:rounded-[2rem] opacity-0 group-hover:opacity-100 transition-opacity duration-500"
          style={{
        ...
        <div className="absolute inset-0 p-6 md:p-8 flex flex-col justify-end">
          ...
          <h3 className="text-xl md:text-2xl font-bold text-white mb-4 md:mb-6">{cap.title}</h3>

          <div className="space-y-2 md:space-y-3 mb-4 md:mb-6">
            {cap.items.map((item: string) => (
              <div key={item} className="flex items-center gap-3">
                <span className={`w-2 h-2 rounded-full bg-${cap.accent}-400 flex-shrink-0`} />
                <span className="text-sm md:text-base text-white/90 font-medium">{item}</span>
              </div>
            ))}
          </div>

          <p className="text-white/70 text-sm leading-relaxed border-t border-white/10 pt-4 opacity-0 group-hover:opacity-100 transition-opacity duration-400">
            {cap.description}
          </p>
