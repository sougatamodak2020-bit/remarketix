          ...
          <div className="relative flex items-center z-10 w-full">
            ...
            <div className="flex w-full justify-end items-center gap-3 min-w-0 mr-4 md:mr-6">
              <div className="hidden lg:flex items-center gap-3">
                {user && (
                  <div className="flex items-center gap-3">
                    {userRole === "admin" && (
                      <motion.button onClick={() => handleNavigation("admin")} whileHover={{ scale: 1.05, y: -1 }} whileTap={{ scale: 0.95 }} className="group relative px-4 py-2 rounded-xl overflow-hidden border border-blue-500/20 bg-blue-500/5 flex items-center gap-2">
                        <div className="absolute inset-0 bg-gradient-to-r from-blue-500/10 to-purple-500/10 opacity-0 group-hover:opacity-100 transition-opacity duration-300" />
                        <Shield className="relative w-4 h-4 text-blue-400" />
                        <span className="relative text-xs font-bold tracking-wide text-blue-200">Admin</span>
                      </motion.button>
                    )}
                    <div className="px-3 py-2 rounded-xl bg-white/5 border border-white/10 backdrop-blur-md flex items-center gap-2">
                      ...
                      <span className="text-xs font-medium text-slate-200 max-w-[100px] truncate">{user.email?.split('@')[0]}</span>
                    </div>
                    <motion.button onClick={handleSignOut} whileHover={{ scale: 1.05, backgroundColor: "rgba(239, 68, 68, 0.1)" }} whileTap={{ scale: 0.95 }} className="p-2 rounded-xl border border-white/5 text-slate-400 hover:text-red-400 hover:border-red-500/30 transition-colors flex items-center justify-center" title="Sign Out">
              ...
              <motion.button onClick={toggleMobileMenu} className="lg:hidden relative p-2.5 rounded-xl border border-white/10 bg-white/5 backdrop-blur-md min-h-[44px] min-w-[44px] flex items-center justify-center overflow-hidden group z-20" whileTap={{ scale: 0.95 }} aria-label={isMobileMenuOpen ? "Close menu" : "Open menu"}>
                ...
                <motion.div animate={{ rotate: isMobileMenuOpen ? 90 : 0 }} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }}>
                  {isMobileMenuOpen ? <X className="relative w-5 h-5 text-white" /> : <Menu className="relative w-5 h-5 text-slate-200" />}
                </motion.div>
              </motion.button>
            </div>
          </div>
          ...
          <motion.div initial={{ opacity: 0, y: -20, scale: 0.98 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -20, scale: 0.98 }} transition={{ duration: 0.3, ease: [0.22, 1, 0.36, 1] }} className="fixed top-[72px] left-4 right-4 z-[9999] lg:hidden max-h-[calc(100vh-90px)] overflow-y-auto rounded-2xl border border-white/10 bg-[#0f172a]/95 backdrop-blur-2xl shadow-2xl shadow-black/50">
            <div className="p-4 space-y-2">
              {NAV_ITEMS.map(({ id, label, icon: Icon }, index) => (
                <motion.button key={id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: index * 0.03, duration: 0.3 }} onClick={() => handleNavigation(id)} className={`nav-item-mobile ${activeView === id ? "active" : ""}`} whileTap={{ scale: 0.98 }}>
                  <Icon className={`w-5 h-5 transition-all duration-300 ${activeView === id ? "text-emerald-400" : "text-slate-500 group-hover:text-emerald-400"}`} />
              ...
              {user && (
                <div className="mt-4 pt-4 border-t border-white/10 space-y-3">
                  <div className="px-4 py-3 rounded-xl bg-white/5 border border-white/10 flex items-center gap-3">
                    <div className="w-8 h-8 rounded-full bg-gradient-to-br from-emerald-500 to-cyan-500 flex items-center justify-center text-white font-bold text-xs">{user.email?.[0].toUpperCase()}</div>
                  ...
                  {userRole === "admin" && (
                    <motion.button onClick={() => handleNavigation("admin")} whileTap={{ scale: 0.98 }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-blue-500/10 border border-blue-500/20 text-blue-300 font-medium hover:bg-blue-500/20 transition-colors">
                      <Shield className="w-5 h-5" />Admin Panel
                    </motion.button>
                  )}
                  <motion.button onClick={handleSignOut} whileTap={{ scale: 0.98 }} className="w-full flex items-center gap-3 px-4 py-3 rounded-xl bg-red-500/10 border border-red-500/20 text-red-300 font-medium hover:bg-red-500/20 transition-colors">
